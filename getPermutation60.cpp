...
//时间复杂度：O(n)
//空间复杂度：O(n)
std::string getPermutation(int n, int k,std::vector<int>& visited,std::string& ret) {
    //当字符串中数字个数等于总个数函数结束（注意不能等于n,因为n每次递归都在递减）
    if(ret.size()==visited.size()-1){
        return ret;
    }
    ...
    ret+=std::to_string(position);
    visited[position]=true;
    //k在第count组的第k（计算出来的k）个
    k=k%num==0 ? num : k%num;
    //循环递归
    return getPermutation(n-1,k,visited,ret);
}
std::string getPermutation(int n, int k) {
    ...
    return getPermutation(n,k,visited,ret);
}
